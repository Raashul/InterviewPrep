...
the tree is linear and the height is in O(n). Therefore, space complexity due to
...
Because we traverse the entire input tree once, the total run time is O(n), where n i
the total number of nodes in the tree.

There is additional space required for the search queue. In the worst case, we have to
insert O(n) nodes in the queue. Therefore, space complexity is O(n)O(n).
